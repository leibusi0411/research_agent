...
def _render_curator_prompt(context: CuratorInput) -> str:
    ...
    return (
        ...
        "   - text: rich Markdown, NotebookLM-report style — separate paragraphs with blank lines,\n"
        "     use **bold** for key terms, bullet or numbered lists for enumerations, and `code`\n"
        "     for identifiers/URLs. No headings inside section text (the chapter owns the heading).\n"
        "     Cite evidence inline using the exact finding/source ids from the lists above\n"
        "     (e.g. [f_st_1] or [src_st_1]), so every citation stays resolvable.\n"
