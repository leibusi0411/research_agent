...
**Web Source Snapshot**:
The source metadata record saved for a Web Source inside task files. The current snapshot stores only the `WebSource` metadata fields (`source_id`, `title`, `url`, `fetched_at`) as a local internal artifact; extracted source text is no longer persisted (see the evolution note in ADR-0021). Snapshots are not included in the Web Report File body.
...
The task-level boundary that keeps Local RAG Research and Web Research as separate execution modes. A Local RAG task does not run Web Research, and the Web Research Executor never touches the Knowledge Base — its tool loop is web-only. Local knowledge enters Web Research only through the Planner's read-only channels: Prior Knowledge (a one-time hybrid retrieval seed before the graph starts, ADR-0046) and the Planner's local knowledge survey (self-directed local_kb_search queries run before the plan is finalised, ADR-0048). Knowledge Deposit does not cross this boundary either: it is an explicit post-hoc copy of a finished Web Report File into the Markdown Vault (followed by a deterministic incremental index refresh), never a channel into a running task.
...
**Research ID Policy**:
The v1 identifier convention for persisted objects. `task_id` uses `task_yyyymmdd_hhmmss_<random6>` and Web subtasks use task-local `st_1`, `st_2`, both assigned by the runtime. Web `source_id` and `finding_id` values are generated by the model through the executor output schema, with the runtime falling back to `src_{subtask_id}` / `f_{subtask_id}` when the model omits them. Local RAG chunks use internal `chunk_<short_hash>` IDs derived from source path, offsets, mtime, and size.
...
**Event Sequence**:
A monotonically increasing integer `seq` assigned by `_emit` to each Research Progress Stream record it emits. Records that do not pass through `_emit` (local-mode and API-originated events) carry no `seq`. Consumers use `seq` to deduplicate events when the EventStream first replays persisted events from `events.jsonl` and then switches to live queue consumption—any event whose `seq` was already yielded during file replay is skipped from the queue, and a missing `seq` is tolerated as `-1`. The counter is per-runner, not global.
_Avoid_: event_id, timestamp ordering

**Runner Registry**:
An in-memory mapping from `task_id` to `StateGraphRunner` held at the API module level. It allows the SSE endpoint (`GET /api/tasks/{task_id}/events`) to look up the live runner for an active task and obtain its `EventStream`. The registry entry is removed via an `on_done` callback invoked when the runner's `run()` method completes. When a task is not found in the registry (already finished or never existed), the SSE endpoint falls back to returning the full `events.jsonl` content as a JSON array rather than an SSE stream.
...
**Debug Trace**:
The diagnostic record of full model prompts and outputs kept for troubleshooting. There is no explicit Debug Trace mode or toggle; full prompts and model outputs are persisted by default under `tasks/<task_id>/artifacts/llm_calls/` (ADR-0008, evolved), beyond what Task History keeps.
...
**Hybrid Retrieval**:
The initial Local RAG retrieval strategy that combines SQLite FTS5 full-text search (keyword) and ChromaDB vector search (semantic), fuses results via Reciprocal Rank Fusion (RRF, k=60), and returns the top-10 fused, traceable Chunks. Falls back to FTS5-only when no embedding client is available. ChromaDB embedding is batched (32 chunks per request) to stay within API limits. Since ADR-0052/0053 the retrieval seam optionally adds Query Rewrite (before recall) and Reranker reordering (after RRF fusion, before truncation); each is enabled by the presence of its model config (`[chat_model.local_summarizer]` slot / `[rerank_model]` section) and falls back to plain hybrid retrieval when unconfigured. Retrieval enhancements beyond these two opt-in layers remain out of scope.
_Avoid_: semantic search, reranking

**Query Rewrite**:
The optional Multi-Query preprocessing step (ADR-0052) in which the `local_summarizer` role model turns the user question into two retrieval variants — one keyword-extraction style, one semantic-descriptive style — in a single LLM call. Every query (original plus variants) runs the full hybrid retrieval and all ranked lists fuse through the same RRF. Enabled by configuring the `[chat_model.local_summarizer]` slot (absent slot = no rewriting, and summarization falls back to the global chat model); it applies only to Local RAG, never to Prior Knowledge or the Planner local survey, and any LLM failure degrades to the original question alone.
_Avoid_: query expansion, search rewriting

**Reranker**:
The optional cross-encoder reordering step (ADR-0053) that scores fused retrieval candidates against the question through a rerank-style HTTP API (`POST {base_url}/rerank`, query + documents in, relevance scores out) and reorders them before the top-k truncation. Configured under `[rerank_model]` — the section's presence enables it (absent section = no rerank layer); it applies only when candidates exceed the display budget, and an API failure degrades to the RRF order without failing the task. It never rewrites or filters content — only the order changes.
...
**PlannerInput**:
The structured input used by Planner. Initial planning receives the original question; Plan Revision receives the original question, the current plan, and research gaps—the Supervisor's `plan_revision_request` and executor outputs do not enter the revision prompt.
...
**Runtime Failure**:
A system-level failure such as invalid configuration, unavailable model provider, schema validation failure, or node code error. LLM call or parse failures are retried blindly once (two attempts total) before raising `llm_call_failed`; an LLM schema validation failure terminates the Web Research Workflow immediately with `schema_validation_failed`, with no repair attempt, and returns an error instead of being routed as a research gap.
_Avoid_: subtask failure, source failure

**Schema Repair**:
A retired mechanism: an internal fallback LLM call that once attempted to fix a role output that failed parsing or schema validation, sending the raw invalid output, validation error, and target schema back to the model. The mechanism was removed entirely with the migration to native function calling and no longer exists anywhere in the runtime. Current behavior: role invocations go through native function calling; an LLM call or parse failure is retried blindly once (two attempts total) before raising `llm_call_failed`; a schema validation failure terminates the Web Research Workflow immediately with `schema_validation_failed`. The original "one repair call per role invocation" behavior is gone.
...
**Core Service**:
The shared application layer that exposes the Capability Surface to interfaces such as Web UI and CLI. It owns research, Local RAG retrieval, Web Research, Knowledge Base Index, and Knowledge Deposit workflows so interfaces do not implement separate behavior. V1 exposes a small use-case surface: `init_config`, `run_local_research`, `run_web_research`, `run_both`, `list_finished_tasks`, `delete_finished_task`, `deposit_web_report`, `get_kb_status`, and `rebuild_kb_index`, plus `run_local_research_unlocked`, `run_web_research_unlocked`, and `acquire_family_lock` for API-layer orchestration. On background-task orchestration the FastAPI layer is no longer a pure thin adapter: it creates the task folder, emits the first event, and manages the lock lifecycle itself (ADR-0037 evolution note).
...
**Tool Gateway**:
The Web Research Runtime interface that validates standard function calls before tool execution. It checks tool existence, input schema validity, and workflow permission, then returns structured tool results; low-level transient retries default to 2. The gateway itself does not record calls—call records land in the task record through executor progress events and `llm_calls` artifacts. V1 applies wide per-call engineering boundaries. No URL deduplication is performed at any stage in v1, neither at the gateway nor at the Curator output stage (consistent with ADR-0031; the older wording in ADR-0004 and ADR-0035 has evolved).
...
**Tasks Page**:
The Web UI page for listing all finished Research Tasks in one mixed `local` and `web` list sorted by `created_at` descending. Clicking a task row expands an inline detail card by calling `GET /api/tasks/{task_id}/result` and rendering the mode-appropriate view (Local Result Page or Web Report Page) in place, without navigating to a separate route. Running tasks are shown through the active Research Progress Stream and Process View, not the task-management list; finished tasks offer a Delete action (running tasks cannot be deleted), and v1 does not include pagination, `error_summary`, or report file error columns here.
